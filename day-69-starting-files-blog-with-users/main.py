...
from forms import CreatePostForm,RegisterUser,LohinUser
...
# CONNECT TO DB
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////Users/air/Desktop/100daysofpython/day-69-starting-files-blog-with-users/instance/posts.db'
db = SQLAlchemy()
db.init_app(app)
...
def login():
    ...
    if request.method=="POST":
        ...
        passwordd= form.passwordd.data
        with app.app_context():
            wanted_object= db.session.execute(db.select(UserDb).where(UserDb.email == emaill)).scalar()
            if wanted_object:
